...
//COMECA AQUI
int main(){
    /** Entrada de parâmetros: IN */
    char IN[] = "arqs_inter.in";
    //char IN[] = "PR_Rio Negro_14-_P.man";
    ...
    FILE *in, *in_c, *out, *out2;

    if( (in=fopen(IN,"r")) == NULL ) printf("Arquivo nao pode ser aberto\n");

/** Para arquivos nomeados com espaço utilizar [^\n] */
    fscanf(in,"%[^\n]s\n", &IN_campo);

    printf("\nArquivo CPQd: %s\n", IN_campo);

    if( (in_c=fopen(IN_campo,"r")) == NULL ) printf("Arquivo nao pode ser aberto\n");
